...
class BinanceOpenOrderTest(unittest.TestCase):
    ...
    def test_init_valid(self):
        # Arrange
        binance_open_order = BinanceOpenOrder(
            ORDER_ID, TRADING_PAIR, TYPE, EXECUTED_QUANTITY, SIDE, TIME)

        # Assert
        self.assertEqual(ORDER_ID, binance_open_order.order_id)
        self.assertEqual(TRADING_PAIR, binance_open_order.trading_pair)
        self.assertEqual(EXECUTED_QUANTITY, binance_open_order.executed_quantity)
    ...
    def test_init_invalid_order_id_exception(self):
        ...
        # Assert
        self.assertRaises(Exception, BinanceOpenOrder, (order_id,
                          TRADING_PAIR, TYPE, EXECUTED_QUANTITY, SIDE, TIME))
