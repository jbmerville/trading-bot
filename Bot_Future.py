...
def get_historic():
    ...
    #api = tweepy.API(auth)
    tweets = client.get_users_tweets(id="1210100315980210176", tweet_fields=['text', 'created_at'], max_results=100)
...
def backtest():
    ...
    for date_tweet in reversed(tweets.index):
        ...
        is_Order, trading_pair, direction, in_level, tp1_level, tp2_level, sl_level, lev_level = isOrder(content)
        if is_Order:
            c = c+1
            print(content)
            print("Direction: "+direction)
